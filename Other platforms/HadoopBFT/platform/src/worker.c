...
static int compute(int argc, char* argv[]) {
	...
	localhost_id = get_worker_id(MSG_host_self());
	...
	if (job.task_status[ti->phase][ti->id] != T_STATUS_DONE) {
		...
		TRY
		{
			if(!job.byzantine_flag[get_worker_id(MSG_host_self())]){
				status = MSG_task_execute(task);
			}else{
				XBT_INFO("%lu -> is acting maliciously", localhost_id);
				status = MSG_task_execute(task);
				XBT_INFO("%lu -> finished its malicious job ", localhost_id);
			}

			if (ti->phase == MAP && status == MSG_OK)
				update_intermediate_result_owner(ti->id, localhost_id);
		}
		CATCH(e)
		{
	...
	if (!job.finished){
		if(job.byzantine_flag[get_worker_id(MSG_host_self())]){
			send(SMS_TASK_DONE_BYZANTINE, 0.0, 0.0, ti, MASTER_MAILBOX);
		}else{
			send(SMS_TASK_DONE_CORRECT, 0.0, 0.0, ti, MASTER_MAILBOX);
		}
	}
